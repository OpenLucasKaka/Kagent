...
- `kagent-doctor --production` fails deployment self-checks
...
- `KAGENT_SERVICE_IDEMPOTENCY_CACHE_PATH` enables SQLite-backed retry
...
- `KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS` restricts which Codex-style
...
- `KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS_BY_SUBJECT` applies stricter
...
- `KAGENT_SERVICE_TRACE_DIR` persists full traces only when explicitly
  configured.
...
- The Kubernetes `NetworkPolicy` permits service ingress on port `8000` only
  ...
  `kagent-access: "true"`. Label trusted ingress gateways,

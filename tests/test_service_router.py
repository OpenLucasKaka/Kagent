...
def test_service_router_runtime_policy_reports_admin_audit_view_without_tokens():
    ...
    assert payload["effective_tool_policy"] == [
        ...
        {"name": "memory_get", "allowed": "false", "approval_required": "true"},
        {"name": "memory_put", "allowed": "false", "approval_required": "true"},
        {"name": "memory_recall", "allowed": "false", "approval_required": "true"},
        {"name": "memory_remember", "allowed": "false", "approval_required": "true"},
        {"name": "memory_search", "allowed": "false", "approval_required": "true"},
        {"name": "memory_upsert", "allowed": "false", "approval_required": "true"},
...
def test_service_router_runtime_resume_treats_prior_dependencies_as_satisfied(
    ...
    url = _mock_public_http_request(monkeypatch, b"dependency satisfied")
    first_status, first_payload = service_router.handle_request(
        "POST",
        "/runtime/run",
        json.dumps(
            {
                "goal": "record then fetch with dependency",
                "plan": {
                    "actions": [
                        {
                            "id": "step-1",
                            "tool": "note",
                            "input": {"text": "dependency already done"},
                            "reason": "record context",
                        },
                        {
                            "id": "step-2",
    ...
    assert resume_status == 200
    assert resume_payload["status"] == "done"
    assert resume_payload["observations"][0]["action_id"] == "step-2"
...
def test_service_router_runtime_status_sanitizes_final_answer_guardrail(tmp_path):
    persist_trace(
        {
            "trace_type": "codex_runtime",
            "run_id": "malformed-guardrail",
            "status": "done",
            "goal": "你是谁",
            "final_answer_guardrail": {
                "applied": "true",
                "reason": {"secret": "guardrail-reason"},
                "original_answer_omitted": ["secret-answer"],
            },
            "observations": [],
        },
        str(tmp_path),
    ...
    status_code, payload = service_router.handle_request(
        "GET",
        "/runtime/runs/malformed-guardrail",
        b"",
        config=ServiceConfig(trace_dir=str(tmp_path)),
    )
    ...
    assert payload["final_answer_guardrail"] == {"applied": "true"}
    assert summary_payload["final_answer_guardrail_applied_count"] == "1"
    assert summary_payload["final_answer_guardrail_reason_counts"] == {}
    assert "secret" not in json.dumps(payload)

...
class StdioRuntimeSession:
    def __init__(
        ...
        *,
        ...
        self.pending_approval_path = _resolve_pending_approval_path(
            pending_approval_path,
            self.memory_path,
            custom_memory_path=custom_memory_path,
        )
    ...
    def _handle_run_request(self, request: Request) -> None:
        ...
        self._start_active_run(
            ...
            allow_live_replan=not bool(
                str(effective_request.get("runtime_plan", "")).strip()
            ),
        )
...
def _resolve_pending_approval_path(
    configured_path: str | None,
    memory_path: str,
    *,
    custom_memory_path: bool,
) -> str:
    if configured_path is not None:
        return configured_path
    if os.environ.get("KAGENT_PENDING_APPROVAL_PATH", "").strip():

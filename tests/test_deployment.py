...
def test_dockerfile_defines_runtime_image_healthcheck_and_non_root_user():
    ...
    assert "kagent-serve" in dockerfile
...
def test_dockerignore_excludes_local_and_generated_artifacts():
    ...
    assert "src/kagent.egg-info" in dockerignore
...
def test_deployment_documentation_is_linked_from_readme():
    ...
    assert "deploy/kubernetes/kagent.yaml" in deployment
    ...
    assert "KAGENT_SERVICE_AUTH_TOKENS" in deployment
    ...
    assert "KAGENT_SERVICE_MAX_GOAL_CHARS" in deployment
    assert "KAGENT_SERVICE_ALLOW_FULL_TRACE_RESPONSE" in deployment
    assert "KAGENT_SERVICE_PROTECT_DIAGNOSTICS" in deployment
    assert "--require-auth" in deployment
    assert "--production" in deployment
    assert "KAGENT_SERVICE_MAX_CONCURRENT_RUNS" in deployment
    assert "KAGENT_SERVICE_IDEMPOTENCY_CACHE_PATH" in deployment
    ...
    assert "workspace_history" in deployment
    ...
    assert "kagent-trace-prune" in deployment
...
def test_environment_example_documents_service_runtime_knobs():
    ...
    assert "KAGENT_SERVICE_HOST=0.0.0.0" in env_example
    assert "KAGENT_SERVICE_PORT=8000" in env_example
    assert "KAGENT_SERVICE_MAX_REQUEST_BYTES=65536" in env_example
    assert "KAGENT_SERVICE_MAX_GOAL_CHARS=4096" in env_example
    assert "KAGENT_SERVICE_AUTH_TOKEN=" in env_example
    assert "KAGENT_SERVICE_AUTH_TOKENS=" in env_example
    assert "KAGENT_SERVICE_RATE_LIMIT_PER_MINUTE=0" in env_example
    assert "KAGENT_SERVICE_MAX_CONCURRENT_RUNS=4" in env_example
    assert "KAGENT_SERVICE_IDEMPOTENCY_CACHE_SIZE=0" in env_example
    assert "KAGENT_SERVICE_IDEMPOTENCY_CACHE_PATH=" in env_example
    assert "KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS=" in env_example
    ...
    assert "KAGENT_LLM_MODEL=" in env_example
    assert "KAGENT_LLM_TIMEOUT_SECONDS=30" in env_example
    assert "KAGENT_LLM_MAX_RETRIES=2" in env_example
    assert "KAGENT_LLM_RETRY_BACKOFF_SECONDS=0.25" in env_example


def test_systemd_unit_runs_service_with_restart_policy():
    unit = Path("deploy/systemd/kagent.service").read_text()

    assert "ExecStart=" in unit
    assert "ExecStartPre=" in unit
    assert "kagent-doctor --production" in unit
    assert "kagent-serve" in unit
...
def test_systemd_unit_defines_trace_state_write_boundary():
    ...
    assert "StateDirectory=kagent" in unit
    ...
    assert "/var/lib/kagent/traces" in deployment
...
def test_systemd_unit_documents_graceful_shutdown_window():
    ...
    assert "KAGENT_SERVICE_RUN_TIMEOUT_SECONDS" in deployment
...
def test_kubernetes_manifest_defines_production_runtime_resources():
    ...
    assert "kagent-doctor" in manifest
    ...
    assert "KAGENT_SERVICE_RATE_LIMIT_PER_MINUTE" in manifest
    assert "KAGENT_SERVICE_MAX_GOAL_CHARS" in manifest
    assert "KAGENT_SERVICE_IDEMPOTENCY_CACHE_SIZE" in manifest
    assert "KAGENT_SERVICE_IDEMPOTENCY_CACHE_PATH" in manifest
    assert "/var/lib/kagent/traces/.idempotency.sqlite3" in manifest
    assert "KAGENT_SERVICE_RUNTIME_ALLOWED_TOOLS" in manifest
...
def test_kubernetes_manifest_defines_availability_and_network_policies():
    ...
    assert 'kagent-access: "true"' in manifest
    assert "\n  egress:\n" in manifest
    ...
    assert "kagent-access" in deployment
    assert "kagent-access" in security
...
def test_kubernetes_manifest_defines_trace_prune_cronjob():
    ...
    assert "name: kagent-trace-prune" in manifest
    ...
    assert "kagent-trace-prune" in manifest
...
def test_prometheus_alert_rules_cover_service_and_agent_health():
    rules_path = Path("deploy/prometheus/kagent-rules.yaml")
    ...
    assert "kagentRuntimeLifecycleFailures" in rules
    ...
    assert "kagentRuntimeSubjectLifecycleApprovalsPending" in rules
    ...
    assert "kagent_request_duration_seconds_bucket" in rules
    assert "kagent_agent_run_duration_seconds_bucket" in rules
    assert "kagent_runtime_run_duration_seconds_bucket" in rules
    assert "kagent_run_status_total" in rules
    assert "kagent_runtime_run_status_total" in rules
    assert "kagent_runtime_run_status_by_auth_subject_total" in rules
    assert "kagent_runtime_resumes_by_auth_subject_total" in rules
    assert "kagent_runtime_approval_required_total" in rules
    ...
    assert 'kagent_requests_by_method_total{method="__unknown__"}' in rules
...
def test_prometheus_servicemonitor_documents_scrape_target():
    monitor_path = Path("deploy/prometheus/kagent-servicemonitor.yaml")
    ...
    assert "name: kagent" in monitor
    assert "app.kubernetes.io/name: kagent" in monitor
...
def test_grafana_dashboard_covers_internal_runtime_operations():
    dashboard_path = Path("deploy/grafana/kagent-dashboard.json")
    deployment = Path("docs/deployment.md").read_text()
    ...
    assert "kagent_responses_total" in payload
    assert "kagent_runtime_run_duration_seconds_bucket" in payload

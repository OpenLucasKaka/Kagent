...
def test_operations_runbook_documents_continuous_iteration_and_failure_triage():
    ...
    assert "sandbox.backend" in runbook
    assert "sandbox.enforced" in runbook
    ...
    assert "optional status fields" in runbook
    ...
    assert "lifecycle_state=waiting_approval" in runbook
    ...
    assert "previous_path" in runbook
    ...
    assert "llm_provider_request_status" in runbook
    assert "llm_provider_status=failed" in runbook
    assert "has_llm_provider_retries=true" in runbook
    assert "runtime_llm_provider_request_retries_total" in runbook
    assert "kagent_runtime_llm_provider_*" in runbook
    ...
    assert "runtime_runs_by_auth_subject_lifecycle_state" in runbook
    assert "runtime_resumes_by_auth_subject" in runbook
    assert "runtime_approvals_by_auth_subject" in runbook
    ...
    assert "kagent_runtime_run_lifecycle_state_by_auth_subject_total" in runbook

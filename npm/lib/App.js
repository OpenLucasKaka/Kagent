...
const FRAMES = ["∙  ", "∙∙ ", "∙∙∙", " ∙∙", "  ∙"];
function KagentInkApp({ React, Ink, runtimeClientFactory = runtime_client_1.runRuntimeGoal, }) {
    const { Box, Text } = Ink;
    ...
    return React.createElement(Box, { flexDirection: "column", paddingX: 1 }, React.createElement(Header, { React, Box, Text }), React.createElement(MessageList, { React, Box, Text, messages }), React.createElement(StatusLine, { React, Text, frame, status, statusText }), React.createElement(PromptLine, { React, Box, Text, cursor, input, disabled: status === "thinking" }));
}
function Header({ React, Box, Text }) {
    return React.createElement(Box, { flexDirection: "column", marginBottom: 1 }, React.createElement(Text, { bold: true, color: "cyan" }, "kagent"), React.createElement(Text, { color: "gray" }, "local agent runtime"));
}
function MessageList({ React, Box, Text, messages }) {
    ...
    return React.createElement(Box, { flexDirection: "column" }, ...recent.map((message, index) => {
        ...
        return React.createElement(Text, { key: `${message.role}-${index}`, color }, `${marker} ${message.text}`);
    }));
}
...
function PromptLine({ React, Box, Text, cursor, disabled, input, }) {
    ...
    return React.createElement(Box, { marginTop: 1 }, React.createElement(Text, { color: disabled ? "gray" : "cyan" }, "› "), input
        ? React.createElement(Text, null, before, React.createElement(Text, { inverse: !disabled }, active), after)
